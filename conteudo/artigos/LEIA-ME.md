...
## 5. Editor visual (jeito fácil)

O projeto vem com um arquivo `.pages.yml` na raiz, que configura o **Pages CMS** — um editor visual gratuito para sites guardados no GitHub.

Para ligar, uma vez só:

1. Acesse [app.pagescms.org](https://app.pagescms.org).
2. Entre com a sua conta do GitHub e autorize o acesso ao repositório `empregadoremdia`.
3. Pronto.

A partir daí, escrever um artigo é preencher um formulário: título, resumo, trilha (lista), assunto (lista), data (calendário), destaque (sim/não) e o texto. Ao salvar, o Pages CMS grava o arquivo no repositório, o GitHub Actions reconstrói e o artigo entra no ar em um ou dois minutos.

A tela de **Configurações do site** edita o `conteudo/site.json` pelo mesmo formulário — é por ali que se troca o WhatsApp, o e-mail e o endereço do Formspree, sem mexer em JSON.

O campo do texto continua sendo markdown simples (`##` para subtítulo, `**negrito**`, hífen para lista, e as caixas `:::`). Isso é proposital: um editor de texto rico reformataria as caixas de destaque e quebraria a formatação delas.

O arquivo `.pages.yml` não participa da geração do site — se você parar de usar o editor, é só ignorá-lo.

---

## 6. Publicar um artigo pelo GitHub (jeito manual)
